...
function requestChannel (opts) {
  ...
  async function requestChannelIfNoneExists () {
    ...
    const isTestnet = /rippletest\.net/.test(opts.server)
...
async function createChannel ({ api, address, secret, peerAddress, publicKey }) {
  ...
  try {
    ...
    // This does not need to be cryptographically secure because it's just meant to
    // avoid overlap between two outgoing channels we are creating at the same time
    const sourceTag = Math.floor(Math.random() * MAX_UINT32)
