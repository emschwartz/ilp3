  ...
  return {
    incoming: function () {
      return async function (ctx, next) {
        ...
        if (!balances[transfer.from]) {
          try {
            balances[transfer.from] = Big(await db.get(`balance ${transfer.from}`))
          } catch (err) {
            if (err.notFound) {
    ...
    outgoing: function () {
      return async function (ctx, next) {
        ...
        if (ctx.fulfillment) {
          balances[transfer.to] = (balances[transfer.to] || Big(0)).plus(transfer.amount)
          await db.put(`balance ${transfer.from}`, balances[transfer.to].toString())
