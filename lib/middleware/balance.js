...
function tracker (opts) {
  ...
  async function ensureRecordExists (account) {
    if (!balances[account]) {
      try {
        const balanceFromDb = Big(await db.get(account))
        // Check that this still isn't set, in case two incoming requests
        // caused us to look up the balance twice
        if (!balances[account]) {
          balances[account] = balanceFromDb
      ...
      return async function (ctx, next) {
        ...
        if (newBalance.lt(minBalance)) {
          ...
          if (account.adjustBalance && !Big(account.adjustBalance).eq(0)) {
            await db.put(transfer.from, balances[transfer.from].toString())
          }
        ...
        try {
          await Promise.all([
            next(),
            db.put(transfer.from, balances[transfer.from].toString())
          ])
        } catch (err) {
    ...
    outgoing: function () {
      return async function (ctx, next) {
        ...
        if (ctx.fulfillment) {
          ...
          await db.put(transfer.to, balances[transfer.to].toString())
        }
      }
    }
  }
