...
function validator () {
  return async function (ctx, next) {
    ...
    if (ctx.fulfillment) {
      ...
      // TODO is this requirement necessary?
      if (fulfillment.length !== 32) {
        const err = new Error(`fulfillment is more than 32 bytes: ${fulfillment.toString('base64')} (condition was: ${condition.toString('base64')})`)
        err.status = 502
        err.code = 'T01' // TODO need error for "we got an invalid fulfillment"
        throw err
      }
