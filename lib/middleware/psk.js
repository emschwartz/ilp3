...
function sender () {
  return async function (ctx, next) {
    ...
    let condition
    if (ctx.quote) {
      // TODO should we use a random condition instead and indicate using other means that it's a quote?
      ...
      condition = hash(fulfillment).toString('base64')
    ...
    ctx.transfer = Object.assign({}, transfer, {
      data,
      condition,
      expiry: transfer.expiry || new Date(Date.now() + DEFAULT_TRANSFER_TIMEOUT).toISOString()
    })
    ...
    try {
      ...
      if (err.code === 'F99') {
        try {
          const destinationAmount = decrypt(sharedSecret, Buffer.from(err.data || '', 'base64'))
          ctx.destinationAmount = destinationAmount
          return
        } catch (err) {
          throw new Error('error decrypting quote response: ' + err.message)
        }
      }
...
function receiver ({ secret }) {
  ...
  return async function (ctx, next) {
    ...
    if (Buffer.from(transfer.condition, 'base64').equals(NULL_CONDITION_BUFFER)) {
      // Quote
      const quoteResponse = encrypt(secret, ctx.transfer.amount).toString('base64')
      const err = new Error('quote response')
      err.status = 418
      err.code = 'F99'
      err.data = quoteResponse
