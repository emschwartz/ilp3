...
function sender (opts) {
  ...
  const randomConditionForQuotes = !!opts.randomConditionForQuotes
  return async function (ctx, next) {
    const debug = Debug('ilp3-psk:sender')
    assert(ctx.sharedSecret, 'sharedSecret is required')
    assert(Buffer.from(ctx.sharedSecret, 'base64').length >= 32, 'sharedSecret must be at least 32 bytes')
    assert(ctx.sourceAmount || ctx.destinationAmount, 'either sourceAmount or destinationAmount is required')

    // TODO accept user data

    if (ctx.quote) {
      await sendQuote(randomConditionForQuotes, ctx, next)
    } else {
      await sendChunkedPayment(ctx, next)
    }
  }
}

async function sendQuote (randomCondition, ctx, send) {
  const debug = Debug('ilp3-psk:sendQuote')
  assert(!ctx.sourceAmount || !ctx.destinationAmount, 'cannot supply both sourceAmount and destinationAmount')
  const condition = (randomCondition ? crypto.randomBytes(32).toString('base64') : NULL_CONDITION)
  const sourceQuote = !!ctx.sourceAmount
  const destinationAmount = ctx.destinationAmount
  const amount = ctx.sourceAmount || STARTING_TRANSFER_AMOUNT
  ...
  ctx.transfer = {
    ...
    data: encrypt(ctx.sharedSecret, headers)
  }
  ...
  try {
    ...
    try {
      const response = decrypt(ctx.sharedSecret, err.data)
      const amountArrived = readUInt64(response, 0)
      if (sourceQuote) {
        ctx.destinationAmount = amountArrived.toString()
      } else {
        const sourceAmount = Big(destinationAmount)
          .div(amountArrived)
          .times(STARTING_TRANSFER_AMOUNT)
          .round(0, 1)
        ctx.sourceAmount = sourceAmount.toString()
      }
      return
    } catch (decryptionErr) {
...
async function sendChunkedPayment(ctx, send) {
  const debug = Debug('ilp3-psk:sendChunkedPayment')
  const sharedSecret = Buffer.from(ctx.sharedSecret, 'base64')
  ...
  if (ctx.destinationAmount) {
    writeUint64(headers, ctx.destinationAmount, 17)
  }
  ...
  while (true) {
    ...
    if (ctx.sourceAmount) {
      amountLeftToSend = Big(ctx.sourceAmount).minus(amountSent)
    } else {
      const amountLeftToDeliver = Big(ctx.destinationAmount).minus(amountDelivered)
      if (amountLeftToDeliver.lte(0)) {
        break
      }
      if (amountSent.gte(0)) {
        const rate = amountDelivered.div(amountSent)
        amountLeftToSend = amountLeftToDeliver.div(rate).round(0)
      } else {
        amountLeftToSend = chunkSize
      }
    }

    if (amountLeftToSend.lte(0)) {
      break
    } else if (amountLeftToSend.lt(chunkSize)) {
      chunkSize = amountLeftToSend
      headers.writeUInt8(TYPE_LAST_CHUNK, 0)
    }

    // TODO accept user data also
    const data = encrypt(sharedSecret, headers)
    const fulfillment = dataToFulfillment(sharedSecret, data)
    const condition = hash(fulfillment)

    debug('sending chunk of: ', chunkSize.toString())
    ctx.transfer = {
      destination: ctx.destination,
    ...
    try {
      await send()
      chunkSize = chunkSize.times(TRANSFER_INCREASE)
      debug('transfer was successful, increasing chunk size to:', chunkSize.toString())
      timeToWait = 0
      try {
        const decryptedData = decrypt(sharedSecret, ctx.data)
      ...
      chunkSize = chunkSize.times(TRANSFER_DECREASE)
...
function receiver ({ secret, notifyEveryChunk }) {
  ...
  return async function (ctx, next) {
    ...
    if (type === TYPE_QUOTE) {
      ...
      ctx.data = encrypt(secret, writeUInt64(Buffer.alloc(8), record.received))
    } else {
