...
function authenticator ({ secret }) {
  ...
  return async (ctx, next) => {
    try {
      ...
      if (!ctx.state.accounts) {
        ctx.state.accounts = {}
      }
      if (!ctx.state.accounts[account]) {
        ctx.state.accounts[account] = {}
      }
      if (!ctx.transfer) {
        ctx.transfer = {}
      }
      ctx.transfer.from = account
      ctx.state.accounts[account].prefix = account
      ctx.state.accounts[account].minBalance = minBalance
    } catch (error) {
